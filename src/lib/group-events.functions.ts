...
export const listEventAttendeeCollabs = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const select = "id,user_id,title,slug,category,description,timeline_text,timeline_mode,starts_on,ends_on,location_mode,compensation_type,status,created_at,resulting_work_id,vouch_count,boost_count, user:profiles!collab_posts_user_id_fkey(id,display_name,username,avatar_url), city:cities!collab_posts_city_id_fkey(name), roles:collab_roles(id,role_name,sort_order)";
    const { data: rows, error } = await supabase
      .from("collab_posts")
      .select(select)
      .in("user_id", ids)
