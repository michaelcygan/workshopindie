...
type PinInput = { roomId: string; collabPostId: string };
...
export const pinCollab = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input: PinInput) => {
    if (!input?.roomId || !input?.collabPostId) throw new Error("roomId and collabPostId required");
    return input;
  })
  .handler(async ({ data, context }) => {
    ...
    if (isHost) {
      ...
      const { error } = await supabaseAdmin.from("instant_room_pins").upsert(
        {
          room_id: data.roomId,
        ...
        { onConflict: "room_id,collab_post_id" },
      );
      if (error) throw new Error(error.message);
    } else {
      // Replace any existing guest pin by this user in this room
      await supabaseAdmin
        .from("instant_room_pins")
        .delete()
        .eq("room_id", data.roomId)
...
export const unpinCollab = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
