 ...
 */
export function formatRoomTitle(title: string | null | undefined, medium?: string | null): string {
  const t = (title ?? "").trim();
  const match = t.match(/^(?:instant\s+workshop|workshop)(?:\s*[:\-–]\s*(.*))?$/i);
  if (match) {
    const captured = match[1]?.trim();
    const label = captured || (medium ? CATEGORIES.find((c) => c.id === medium)?.label : undefined);
