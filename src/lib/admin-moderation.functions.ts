...
export const upsertModerationTerm = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator(
    (d: { id?: string; term: string; severity: "block" | "warn"; notes?: string | null }) => d,
  )
  .handler(async ({ data, context }) => {
    ...
    if (data.id) {
      await admin
        ...
        .update({ term: data.term, severity: data.severity, notes: data.notes ?? null })
        .eq("id", data.id);
    } else {
      await admin
        .from("moderation_terms")
        .insert({ term: data.term, severity: data.severity, notes: data.notes ?? null });
    }
    await logAdminAction(context.supabase, "mod.term.upsert", "moderation_term", null, {
      term: data.term,
...
export const listModerationEvents = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    return (events ?? []).map((e) => ({
      ...e,
      profile: e.user_id ? (pMap.get(e.user_id) ?? null) : null,
    }));
  });
