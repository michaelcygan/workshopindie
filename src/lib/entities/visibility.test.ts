...
describe("public referenceability", () => {
  it("only lets published + public Works into public context", () => {
    ...
    expect(isWorkPubliclyReferenceable({ status: "published", visibility: "unlisted" })).toBe(
      false,
    );
  ...
  it("keeps group-only and unlisted Events out of public context", () => {
    expect(
      isEventPubliclyReferenceable({ visibility: "public", deleted_at: null }, publicGroup),
    ).toBe(true);
    expect(
      isEventPubliclyReferenceable({ visibility: "group_only", deleted_at: null }, publicGroup),
    ).toBe(false);
    expect(
      isEventPubliclyReferenceable({ visibility: "unlisted", deleted_at: null }, publicGroup),
    ).toBe(false);
    expect(
      isEventPubliclyReferenceable({ visibility: "public", deleted_at: "x" }, publicGroup),
    ).toBe(false);
  });
  ...
  it("inherits the parent Group's visibility for Events", () => {
    expect(
      isEventPubliclyReferenceable(
        ...
        { visibility: "unlisted", deleted_at: null },
      ),
    ).toBe(false);
    expect(isEventPubliclyReferenceable({ visibility: "public", deleted_at: null }, null)).toBe(
      false,
    );
  });
