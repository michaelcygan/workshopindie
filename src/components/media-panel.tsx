...
export type RoomViewMode = "chat" | "tools" | "gallery" | "collabs" | "posts" | "links";
...
export function MediaPanel({
  ...
  const [loungeGateOpen, setLoungeGateOpen] = useState(false);
  ...
  const remainingMinutes = audioAccess?.monthlyLimit != null
    ? audioAccess.monthlyLimit - audioAccess.minutesUsed
    : null;
  ...
  return (
    <section className="rounded-3xl border border-border/60 bg-surface/70 backdrop-blur-md p-4 shadow-soft">
      ...
      <PlusGate open={loungeGateOpen} onOpenChange={setLoungeGateOpen} reason="audio_limit" />
    </section>
  );
}
...
function LoungeAudioStrip({
  ...
  return (
    <div className="space-y-2">
      {api.reconnecting && (
        <div className="w-full inline-flex items-center justify-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-medium bg-amber-500/15 text-amber-900">
