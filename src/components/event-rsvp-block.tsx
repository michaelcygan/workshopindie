...
export function EventRsvpBlock({
  ...
  async function commit(s: "going" | "declined") {
    ...
    try {
      ...
      toast.success(
        s === "going"
          ? isFull && waitlistEnabled ? "You're on the waitlist." : "You're in. See you there."
          : "Marked can't make it.",
      );
    } catch (e) {
  ...
  return (
    <div className="rounded-3xl border border-border bg-surface p-5 shadow-soft">
      <div className="mb-3 flex items-center justify-between">
        ...
        {status === "going" && (
          <span className="rounded-full bg-emerald-500/10 px-2 py-0.5 text-[11px] font-medium text-emerald-700">You're going</span>
        )}
      </div>
      <div className="grid grid-cols-2 gap-2">
        ...
        >
          <Check className="mr-1.5 h-4 w-4" /> I'm in
        </Button>
        <Button
          type="button"
