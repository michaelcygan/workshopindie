...
export function VouchRow({
  ...
  const unvouch = useServerFn(unvouchWork);
  ...
  const mut = useMutation({
    mutationFn: async () => {
      if (viewerVouched) await unvouch({ data: { workId } });
      else await vouch({ data: { workId } });
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["gallery"] });
      qc.invalidateQueries({ queryKey: ["work-vouchers", workId] });
  ...
  function handleClick(e: React.MouseEvent) {
    ...
    if (isCreator) {
      toast("You can't vouch for your own Work");
      return;
    }
    mut.mutate();
  }
  ...
  if (followedVouchers.length > 0) {
    const first =
      followedVouchers[0].profile?.display_name ??
      followedVouchers[0].profile?.username ??
      "Someone";
    const rest = vouchCount - 1;
    label =
      rest > 0
        ? `Vouched by ${first} + ${rest} ${rest === 1 ? "other" : "others"}`
        : `Vouched by ${first}`;
...
export type VouchersByWork = Map<string, Voucher[]>;

export function useVouchersForWorks(workIds: string[]): {
  data: VouchersByWork;
  isLoading: boolean;
} {
  const { user } = useAuth();
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["work-vouchers-batch", key, user?.id ?? null],
    enabled: workIds.length > 0,
    queryFn: async (): Promise<VouchersByWork> => {
      const { data: rows, error } = await supabase
        .from("work_vouches")
        .select("work_id,user_id,created_at")
        .in("work_id", workIds)
      ...
      const map: VouchersByWork = new Map();
      for (const r of (rows ?? []) as Array<{ work_id: string; user_id: string }>) {
      ...
      for (const [k, list] of map) {
        map.set(
          ...
          list
            .slice()
            .sort((a, b) => Number(b.follows_viewer) - Number(a.follows_viewer)),
        );
      }
      return map;
    },
