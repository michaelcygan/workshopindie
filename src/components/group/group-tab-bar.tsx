...
import { Calendar, Info, LayoutGrid, Megaphone, Plus, Radio, Sparkles, Users } from "lucide-react";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
export function GroupTabBar({
  ...
  return (
    <div className="sticky top-14 z-20 -mx-4 mt-8 flex items-center gap-1.5 overflow-x-auto border-b border-border bg-background/85 px-4 backdrop-blur md:-mx-6 md:px-6 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
      <div className="flex flex-1 items-center gap-1.5 md:flex-wrap">
        {items.map((t) => {
          const TIcon = t.icon;
      ...
      {/* Trailing create menu — always one click from any tab. */}
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button
            variant="ghost"
            size="sm"
            className="ml-auto shrink-0 rounded-full gap-1"
