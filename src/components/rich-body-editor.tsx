...
export type RichBodyEditorProps = {
  ...
  // ---- Presentation configuration (defaults reproduce the Blog composer) ----
  /** Small uppercase label in the tool stripe. */
  label?: string;
  /** Pill next to the label; pass null to hide it. */
  badge?: string | null;
  /** Placeholder for the first text segment. */
  placeholder?: string;
  /** Storage bucket for inline image uploads. */
  uploadBucket?: "covers" | "work-covers";
  /** Show the word count / reading time readout. */
  showWordCount?: boolean;
  /** Tall writing surface when the body is a single text segment. */
  soloMinHeight?: boolean;
  /** Sticky offset classes for the tool stripe. */
  stickyOffsetClass?: string;
  /** Overrides the left-hand helper line under the surface. */
  helperText?: string;
};
 ...
 */
export function RichBodyEditor({ value, onChange, readOnly, onDirty, onRequestEntityInsert, onBusyChange, label = "Body", badge = "Markdown-light", placeholder = "Write your post…", uploadBucket = "covers", showWordCount = true, soloMinHeight = true, stickyOffsetClass = "top-11 md:top-14", helperText }: RichBodyEditorProps) {
  ...
  async function uploadImageFile(file: File): Promise<string | null> {
    ...
    return uploadToBucket(uploadBucket, user.id, upload);
  }
  ...
  return (
    <div className="rounded-2xl border border-border bg-surface p-3 md:p-4">
      <div
        className={cn(
          "sticky z-10 -mx-3 -mt-3 mb-2 flex flex-wrap items-center gap-2 border-b border-border bg-surface px-3 pb-2 pt-3 md:-mx-4 md:-mt-4 md:px-4 md:pb-2 md:pt-4",
          stickyOffsetClass,
        )}
      >
        ...
        {segments.map((seg, i) =>
          seg.type === "embed" ? (
            ...
            <AutoTextarea
              ...
              minHeight={soloMinHeight && onlyText ? 320 : 48}
              onFocus={() => {
                activeIdx.current = i;
              }}
      ...
      <Dialog open={linkOpen} onOpenChange={setLinkOpen}>
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-3">
            <div>
              <Label htmlFor="rb-link-text">Text</Label>
              <Input
                id="rb-link-text"
                value={linkText}
            ...
            <div>
              <Label htmlFor="rb-link-url">URL</Label>
              <Input
                id="rb-link-url"
                value={linkUrl}
      ...
      >
        <DialogContent className="sm:max-w-md">
          ...
          <div>
            <Label htmlFor="rb-embed-url">URL</Label>
            <Input
              id="rb-embed-url"
              value={embedUrl}
      ...
      >
        <DialogContent className="flex max-h-[85dvh] flex-col overflow-hidden sm:max-w-lg">
          ...
          <div className="-mx-1 min-h-0 flex-1 space-y-3 overflow-y-auto px-1">
            ...
            <div>
              <Label htmlFor="rb-image-url">Image URL</Label>
              <Input
                id="rb-image-url"
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <div>
                <Label htmlFor="rb-image-alt">Alt text</Label>
                <Input
                  id="rb-image-alt"
              ...
              <div>
                <Label htmlFor="rb-image-credit">Credit</Label>
                <Input
                  id="rb-image-credit"
            ...
            <div>
              <Label htmlFor="rb-image-caption">Caption</Label>
              <Input
                id="rb-image-caption"
            ...
            <div>
              <Label htmlFor="rb-image-link">Click-through link (optional)</Label>
              <Input
                id="rb-image-link"
          ...
          <div className="-mx-1 min-h-0 flex-1 space-y-4 overflow-y-auto px-1">
            ...
            <div className="flex items-end gap-2">
              <div className="flex-1">
                <Label htmlFor="rb-gallery-url">Or add an image URL</Label>
                <Input
                  id="rb-gallery-url"
            ...
            <div>
              <Label htmlFor="rb-gallery-caption">Caption</Label>
              <Input
                id="rb-gallery-caption"
