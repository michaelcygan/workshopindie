...
import { MapPin, Calendar, Megaphone, Users, Plus, X } from "lucide-react";
...
function CityPage() {
  ...
  useDocumentMeta({
    title: city?.name,
    description: city ? `Workshops, meetups, and creators in ${city.name}.` : undefined,
  });
  ...
  const { data: workshops = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from("workshops")
        .select("id,title,slug,category,starts_at,location_type,location_text,confirmed_count,participant_cap,status")
        .eq("city_id", city!.id).in("status", ["open", "check_in", "active"])
        .order("starts_at", { ascending: true, nullsFirst: false }).limit(8);
      return data ?? [];
    },
  });
  ...
  const { data: works = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from("works")
        .select("id,title,slug,category,cover_url,source_type,like_count,save_count,view_count,published_at, work_credits(role_label,sort_order, profiles(display_name,username))")
      ...
      return ((data ?? []) as any[]).map<WorkCardData>((r) => ({
        ...
        credits: (r.work_credits ?? []).sort((a: any, b: any) => a.sort_order - b.sort_order)
          .map((c: any) => ({ display_name: c.profiles?.display_name ?? null, username: c.profiles?.username ?? null })),
      }));
    },
  });
  ...
  const { data: collabs = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from("collab_posts")
        ...
        .eq("city_id", city!.id).eq("status", "open").order("created_at", { ascending: false }).limit(6);
      return data ?? [];
    },
  });
  ...
  const { data: creators = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from("profiles")
        .select("id,username,display_name,avatar_url,headline,creator_status,work_count")
        .eq("city_id", city!.id).order("work_count", { ascending: false }).limit(12);
      return data ?? [];
    },
  });
  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-8 md:px-6 md:py-12">
          ...
          <div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {workshops.map((w: any) => (
              <Link key={w.id} to="/workshops/$slug" params={{ slug: w.slug }} className="rounded-2xl border border-border bg-surface p-4 transition hover:shadow-soft">
                ...
                <h3 className="mt-2 font-display text-base text-ink line-clamp-2">{w.title}</h3>
                <p className="mt-1 text-xs text-ink-muted inline-flex items-center gap-1">
                  <Calendar className="h-3 w-3" /> {w.starts_at ? new Date(w.starts_at).toLocaleDateString([], { month: "short", day: "numeric" }) : "TBD"}
                  · <Users className="h-3 w-3" /> {w.confirmed_count}{w.participant_cap ? `/${w.participant_cap}` : ""}
                </p>
              </Link>
            ))}
          </div>
      ...
      {works.length > 0 && (
        <section className="mt-10">
          <h2 className="font-display text-2xl text-ink">Made here</h2>
          ...
          <div className="mt-3 grid gap-3 sm:grid-cols-2">
            {collabs.map((c: any) => (
              <Link key={c.id} to="/collab/$slug" params={{ slug: c.slug }} className="rounded-2xl border border-border bg-surface p-4 transition hover:shadow-soft">
                ...
                <h3 className="mt-2 font-display text-base text-ink line-clamp-2">{c.title}</h3>
                {c.description && <p className="mt-1 text-sm text-ink-soft line-clamp-2">{c.description}</p>}
              </Link>
            ))}
          </div>
        </section>
          ...
          <div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {creators.map((p: any) => (
              <Link key={p.id} to="/u/$username" params={{ username: p.username ?? "" }}
                className="flex items-center gap-3 rounded-2xl border border-border bg-surface p-3 transition hover:shadow-soft">
