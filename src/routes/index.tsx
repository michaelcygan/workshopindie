...
function GalleryControls({
  ...
  // Auto-scroll loop (mobile only). 30s per loop → speed scales with content width.
  useEffect(() => {
    if (!isMobile) return;
    ...
    const loop = (now: number) => {
      ...
      if (!s.paused && !s.dragging) {
        ...
        if (half > 0) {
          const pxPerMs = half / 30000; // full half-loop in 30s
  ...
  return (
    <div className="flex flex-col gap-3 md:flex-row md:flex-wrap md:items-center md:justify-between">
      {isMobile ? (
        <div
          ref={scrollerRef}
          className="relative overflow-x-auto rounded-full border border-border bg-surface p-1 shadow-soft scrollbar-none touch-pan-x cursor-grab active:cursor-grabbing"
